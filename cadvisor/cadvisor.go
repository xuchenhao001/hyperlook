...
func init() {
	...
	for name, defaultValue := range flagOverrides {
		if f := flag.Lookup(name); f != nil {
			...
			log.Printf("Expected cAdvisor flag %q not found", name)
		}
	}
}
...
func (cc *cadvisorClient) exportHTTP(address string, port uint) error {
	...
	if port > 0 {
		...
		go func() {
			...
			for err != nil {
				log.Printf("Failed to register cAdvisor on port %d, retrying. Error: %v", port, err)
...
func (cc *cadvisorClient) getFsInfo(label string) (cadvisorapiv2.FsInfo, error) {
	...
	if len(res) > 1 {
		log.Printf("More than one filesystem labeled %q: %#v. Only using the first one", label, res)
	}

...
// jiangnan
const podPath = "/api/v1/namespaces/%s/pods"
...
func (vm *DockerVM) Start(ctxt context.Context, ccid ccintf.CCID,
	...
	//==========================jiangnan==========================
	exist, err := vm.findImage(client, imageID)
	if err != nil {
		return err
	}
	if !exist {
		dockerLogger.Debug("chaincode image not found, try to create")
		if builder != nil {
	...
	namespace := strings.Split(ccid.PeerID, ".")[1]
	request := make(map[string]interface{})
	request["cpu"] = "40m"
	request["memory"] = "200Mi"
	resources := ResourceRequirements{}
	resources.Requests = request
	envs := make([]EnvVar, len(env))
	for index, value := range env {
		envObject := EnvVar{}
		envObject.Name = strings.Split(value, "=")[0]
		envObject.Value = strings.Split(value, "=")[1]
		envs[index] = envObject
	}
	reg := regexp.MustCompile("[^-a-z0-9]")
	containerSpec := new(Container)
	containerName := ccid.NetworkID+"-"+strings.Split(ccid.PeerID, ".")[0]+"-"+ ccid.GetName()
	containerSpec.Name = reg.ReplaceAllString(containerName, "-")
	containerSpec.Image = imageID + ":latest"
	...
	containers[0] = *containerSpec
	podSpec.Containers = containers
	nodeSelector := make(map[string]string)
	nodeSelector["bc"] = "true"
	nodeSelector["beta.kubernetes.io/arch"] = "amd64"
	podSpec.NodeSelector = nodeSelector
	typeMeta := TypeMeta{}
	typeMeta.APIVersion = "v1"
	typeMeta.Kind = "Pod"
	objectMeta := ObjectMeta{}
	objectMeta.Name = containerID
	objectMeta.Namespace = namespace
	pod := Pod{}
	pod.ObjectMeta = objectMeta
	pod.Spec = podSpec
	pod.TypeMeta = typeMeta
	jsons, errs := json.Marshal(pod)
	if errs != nil {
		dockerLogger.Errorf("change struct to json error , because: %s",errs.Error())
	}
	...
	url := baseURL + path
	var input io.Reader
	bodyContent, _ := json.Marshal(pod)
	input = bytes.NewBuffer(bodyContent)
	//x509.Certificate.
	resp := &Pod{}
	dockerLogger.Debugf("Call k8s rest api to start chaincode pod")
	err1 := callK8sRest(url, "POST", input, resp)
	if err1 != nil {
		dockerLogger.Errorf("Error start pod in k8s image builder for image <%s> (container id <%s>), "+
			"because of <%s>", imageID, containerID, err1)
	}
/*
...
// jiangnan
func (vm *DockerVM) findImage(client dockerClient, imageID string) (bool, error) {
...
// jiangnan
func callK8sRest(url string,method string,inputObject io.Reader, respObject interface{}) error {
        pool := x509.NewCertPool()
        caCertPath := "/var/run/secrets/kubernetes.io/serviceaccount/ca.crt"
        caCrt, err := ioutil.ReadFile(caCertPath)

        if err != nil {
                dockerLogger.Errorf("Read k8s ca.crt error:%s", err)
                return err
        }
        ...
        reqest, _ := http.NewRequest(method, url, inputObject)
        reqest.Header.Set("Authorization", "Bearer "+readFile("/var/run/secrets/kubernetes.io/serviceaccount/token"))
        resp, err := client.Do(reqest)
        if err != nil {
                dockerLogger.Errorf("Call k8s restapi error! Because %s",err.Error())
                return err
        }
        defer resp.Body.Close()
        if resp.StatusCode >= 300 {
		dockerLogger.Errorf("Create Pod call restapi respones code is not OK!")
                return errors.New("resp.StatusCode is " + strconv.Itoa(resp.StatusCode))
        }

        body, err := ioutil.ReadAll(resp.Body)
        if err != nil {
                dockerLogger.Errorf("Call k8s restapi respones error,  because: %s",err.Error())
                return err
        }
        return json.Unmarshal(body, respObject)
}
...
func readFile(path string) string {
	...
	//dockerLogger.Debugf("The file's path is %s, Context: %s",path,string(fd))
	return string(fd)
}
